    ...
    if (newOptions.method === 'POST' || newOptions.method === 'PUT' || newOptions.method === 'DELETE') {
        if (!(newOptions.body instanceof FormData)) {
            if (typeof newOptions.body === 'object' && !Array.isArray(newOptions.body)) {
                const body = __rest(newOptions.body, []);
                delete body.loadingId;
                newOptions.body = body;
            }
            newOptions.headers = Object.assign({ Accept: 'application/json', 'Content-Type': 'application/json; charset=utf-8' }, newOptions.headers);
            newOptions.body = JSON.stringify(newOptions.body);
        }
        else {
            // newOptions.body is FormData
            newOptions.headers = Object.assign({ Accept: 'application/json' }, newOptions.headers);
        }
    }
    //
    newOptions.headers = Object.assign({}, newOptions.headers, { 'Access-Control-Allow-Origin': '*', sessionId: localStorage.getItem('sessionId'), language: localStorage.getItem('intlLang') });
    return fetch(url, newOptions)
        .then((response) => (changeLoadingStatus(response, loadingId)))
        .then(checkStatus)
        .then((response) => {
